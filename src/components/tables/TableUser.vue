<template>
  <v-row no-gutters>
    <v-col cols="12">
      <!-- Input Search -->
      <v-text-field class="mt-2" clearable color="tradewind400" density="comfortable" label="Buscar usuario"
    ...
    <v-data-table :headers="props.headers" :items="props.items" :search="search" :items-per-page="10"
      :sort-by="[{ key: 'status', order: 'desc' }]" ref="tableUser">
      <template v-slot:top>
        <v-toolbar class="rounded-lg" color="tradewind50" density="comfortable" flat>
          ...
          <v-dialog v-model="dialog" persistent max-width="600px">
            ...
            <AddFormUser :form-title="formTitle" :data-form="editedItem" :modal-close="close" :modal-save="save" />
          </v-dialog>
          <!-- Dialog Add/Edit -->
          <!-- Dialog delete -->
          <v-dialog v-model="dialogDelete" max-width="600px" persistent>
            <ModalDelete form-title="Eliminar usuario" type-delete="a" :data-form="editedItem"
